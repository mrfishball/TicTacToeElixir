defmodule GameMaker do
  ...
  def game_menu() do
    IOBuilder.output(Messages.game_menu, MessageFlags.menu)
    Messages.select()
    |> IOBuilder.input(MessageFlags.request)
    |> game_mode()
  end
  ...
  def game_mode(choice) do
    cond do
      ...
      true ->
        IOBuilder.output(Messages.invalid_entry, MessageFlags.error)
        game_menu()
    end
  end
  ...
  defp computer_type_menu({computer_name, _token} = payload) do
      IOBuilder.output(Messages.computer_choice_menu(computer_name), MessageFlags.menu)
      Messages.select()
      |> IOBuilder.input(MessageFlags.request)
  ...
  def set_player_name(player_number) do
    input = IOBuilder.input(Messages.player_name(player_number), MessageFlags.request)
  ...
  def set_player_symbol(player_name) do
    input = IOBuilder.input(Messages.player_symbol(player_name), MessageFlags.request)
    case !empty_input?(input) do
